...
jQuery(document).ready(function($) {
    ...
    const strings = {
        ...
        drop_here: elearningQuiz.strings?.drop_here || (isGreek ? 'Τοποθετήστε εδώ' : 'Drop here'),
    ...
    function initializeQuiz() {
        ...
        // Answer change handlers (THIS IS WHERE YOUR EXISTING HANDLER IS)
        ...
        // ADD THE YES/NO SPECIFIC HANDLER HERE (OPTIONAL - see note below)
        // This is actually optional because the handler above already covers radio buttons
        // But if you want explicit handling for yes/no questions, add it here:
        $(document).on('change', '.quiz-question[data-question-type="yes_no"] input[type="radio"]', function() {
            ...
            console.log('Yes/No answer selected:', $(this).val());
        });
    ...
    // Fix for radio button state persistence issue
    function fixRadioButtonStates() {
        // When navigating between questions, clear radio states from hidden questions
        $('.quiz-question:not(.active)').find('input[type="radio"]').each(function() {
            // Store the value if it was checked
            const $radio = $(this);
            const $question = $radio.closest('.quiz-question');
            const questionIndex = $question.data('question-index');
            
            if ($radio.is(':checked')) {
                // Store in data attribute instead of keeping it checked
                $question.data('saved-answer-' + questionIndex, $radio.val());
                // Uncheck it when question is not active
...
function debugAnswerTracking() {
    console.log('=== ANSWER TRACKING DEBUG ===');
    console.log('Total questions displayed:', $('.quiz-question').length);
    console.log('Saved answers:', currentQuiz.answers);
    
    $('.quiz-question').each(function(displayIndex) {
        const $q = $(this);
        const actualIndex = parseInt($q.data('question-index'));
        const type = $q.data('question-type');
        const hasAnswer = currentQuiz.answers[actualIndex] !== undefined;
        
        console.log(`Question ${displayIndex + 1} (actual index: ${actualIndex}, type: ${type}): ${hasAnswer ? 'ANSWERED' : 'UNANSWERED'}`);
        
        if (hasAnswer) {
            console.log(`  Answer:`, currentQuiz.answers[actualIndex]);
        }
    });
    console.log('=============================');
}
...
debugMatchingAnswers();
    ...
    function handleSubmitQuiz() {
        ...
        $('.quiz-question').each(function(index) {
            const $question = $(this);
            const actualQuestionIndex = parseInt($question.data('question-index')); // Use actual question index, not display index
            const questionType = $question.data('question-type');
            
            let hasAnswer = false;
            
            // Check if we have a saved answer for this question using the actual index
            if (currentQuiz.answers[actualQuestionIndex] !== undefined && currentQuiz.answers[actualQuestionIndex] !== null) {
                hasAnswer = true;
            } else {
                // Double-check by looking at the actual form inputs
                switch (questionType) {
            ...
            if (!hasAnswer) {
                unanswered.push(index + 1); // Use display index (index + 1) for user-friendly message
            }
        });
    ...
    function showQuestion(questionIndex) {
        ...
        // First, clear all radio states from non-active questions
        $('.quiz-question').each(function() {
            const $q = $(this);
            if (!$q.hasClass('active')) {
                $q.find('input[type="radio"]').prop('checked', false);
            }
        });
    ...
    function clearRadioConflicts() {
        $('.quiz-question').each(function() {
            ...
            if (questionType === 'true_false') {
                // Ensure each true/false question has unique radio group
                $question.find('input[type="radio"]').each(function() {
                    const $radio = $(this);
                    const currentName = $radio.attr('name');
                    // Make sure the name includes the question index
                    if (!currentName.includes('[' + questionIndex + ']')) {
                        $radio.attr('name', 'questions[' + questionIndex + '][answer]');
                    }
                });
            }
        });
    ...
    function saveCurrentAnswer() {
        ...
        switch (questionType) {
            ...
            case 'true_false':
                ...
                if (tfAnswer !== undefined && tfAnswer !== null) {
                    answer = tfAnswer;
                }
                console.log('True/False answer saved:', answer, 'for question:', questionIndex);
                break;
                
            case 'yes_no':
                const yesNoAnswer = $currentQuestion.find('input[type="radio"]:checked').val();
                if (yesNoAnswer !== undefined && yesNoAnswer !== null) {
                    answer = yesNoAnswer;
                }
                console.log('Yes/No answer saved:', answer, 'for question:', questionIndex);
                break;
            ...
            case 'column_sorting':
                // CRITICAL FIX: Add column sorting case
                const column1Items = [];
                const column2Items = [];
                
                // Get items from column 1
                $currentQuestion.find('.column-dropzone[data-column-index="1"] .sortable-item').each(function() {
                    const itemText = $(this).data('item-text') || $(this).find('.item-text').text();
                    if (itemText) {
                        column1Items.push(itemText);
                    }
                });
                
                // Get items from column 2
                $currentQuestion.find('.column-dropzone[data-column-index="2"] .sortable-item').each(function() {
                    const itemText = $(this).data('item-text') || $(this).find('.item-text').text();
                    if (itemText) {
                        column2Items.push(itemText);
                    }
                });
                
                // Only save if there are items distributed
                if (column1Items.length > 0 || column2Items.length > 0) {
                    answer = {
                    ...
                    $currentQuestion.find('.column-sorting-answer[data-column="1"]').val(JSON.stringify(column1Items));
                    $currentQuestion.find('.column-sorting-answer[data-column="2"]').val(JSON.stringify(column2Items));
                    
                    console.log('Column sorting saved - Column 1:', column1Items, 'Column 2:', column2Items);
                } else {
                    answer = null;
                }
                break;
        ...
        if (answer !== null && answer !== undefined) {
            ...
            console.log('Current answers object:', currentQuiz.answers);
        } else {
            if (answer === null) {
                delete currentQuiz.answers[questionIndex];
            }
            console.log('No answer or null answer for question:', questionIndex);
        }
    }
    ...
    function loadSavedAnswer(questionIndex) {
        ...
        const questionType = $question.data('question-type');
        const actualQuestionIndex = $question.data('question-index'); // Get the actual question index
        const savedAnswer = currentQuiz.answers[actualQuestionIndex]; // Use actual index
        ...
        switch (questionType) {
            ...
            case 'true_false':
                // Only check the radio in the current question
                $question.find(`input[type="radio"][value="${savedAnswer}"]`).prop('checked', true)
                    .closest('.option-label').addClass('selected');
                break;
                
            case 'yes_no':  // ADD THIS NEW CASE
                $question.find(`input[type="radio"][value="${savedAnswer}"]`).prop('checked', true)
                    .closest('.option-label').addClass('selected');
                break;
            ...
            case 'column_sorting':
                if (savedAnswer && typeof savedAnswer === 'object') {
                    const column1Items = savedAnswer.column1 || [];
                    const column2Items = savedAnswer.column2 || [];
                    
                    // Clear current distribution
                    $question.find('.sortable-item').remove();
                    
                    // Add items to column 1
                    const $column1 = $question.find('.column-dropzone[data-column-index="1"]');
                    column1Items.forEach(itemText => {
                        // Find the original item data
                        const $originalItem = $question.find('.column-sorting-correct-data');
                        const column1Correct = JSON.parse($originalItem.attr('data-column1-correct') || '[]');
                        const column2Correct = JSON.parse($originalItem.attr('data-column2-correct') || '[]');
                        
                        let correctColumn = 'unknown';
                        if (column1Correct.includes(itemText) && column2Correct.includes(itemText)) {
                            correctColumn = 'both';
                        } else if (column1Correct.includes(itemText)) {
                            correctColumn = '1';
                        } else if (column2Correct.includes(itemText)) {
                            correctColumn = '2';
                        }
                        
                        const itemHtml = '<div class="sortable-item" draggable="true" data-item-text="' + itemText + '" data-original-column="' + correctColumn + '">' +
                    ...
                    const $column2 = $question.find('.column-dropzone[data-column-index="2"]');
                    column2Items.forEach(itemText => {
                        // Find the original item data
                        const $originalItem = $question.find('.column-sorting-correct-data');
                        const column1Correct = JSON.parse($originalItem.attr('data-column1-correct') || '[]');
                        const column2Correct = JSON.parse($originalItem.attr('data-column2-correct') || '[]');
    ...
    function initializeDragAndDrop() {
        ...
        $('.column-dropzone').droppable({
            ...
            drop: function(event, ui) {
                ...
                $item.detach().css({top: 0, left: 0}).appendTo($dropZone);
                
                // Update the answer - CRITICAL: Call the function
                updateColumnSortingAnswer($question);
                
                // Provide feedback
                const itemText = $item.data('item-text') || $item.find('.item-text').text();
                const columnIndex = $dropZone.data('column-index');
                const columnTitle = $dropZone.siblings('.column-title').text() || 'Column ' + columnIndex;
                const message = isGreek ? 
                    `${itemText} μετακινήθηκε στη στήλη ${columnTitle}` : 
                    `${itemText} moved to column ${columnTitle}`;
                announceToScreenReader(message);
            }
        });
        
        // Function to update column sorting answer
        function updateColumnSortingAnswer($question) {
            // Get items in each column
            const column1Items = [];
            const column2Items = [];
            
            $question.find('.column-dropzone[data-column-index="1"] .sortable-item').each(function() {
                column1Items.push($(this).data('item-text'));
            });
            
            $question.find('.column-dropzone[data-column-index="2"] .sortable-item').each(function() {
                column2Items.push($(this).data('item-text'));
            });
            
            // Update hidden inputs
            $question.find('.column-sorting-answer[data-column="1"]').val(JSON.stringify(column1Items));
            $question.find('.column-sorting-answer[data-column="2"]').val(JSON.stringify(column2Items));
        }
        ...
        $(document).on('click', '.remove-match', function(e) {
            ...
            // CHANGED: Updated placeholder from "Αφήστε εδώ" to "Τοποθετήστε εδώ"
            $dropZone.html('<span class="drop-placeholder">Τοποθετήστε εδώ</span>').removeClass('has-item');
...
/* Continue from Part 2 */
    ...
    function initializeMobileTapInterface() {
        ...
        // === COLUMN SORTING TAP INTERFACE ===
        $(document).on('click', '.sortable-item', function(e) {
            ...
            // Check if this is actually a column sorting question
            if ($question.data('question-type') !== 'column_sorting') return;
            
            const $currentColumn = $item.closest('.column-dropzone');
            const currentColumnIndex = $currentColumn.data('column-index');
            
            // Toggle to the other column
            const newColumnIndex = currentColumnIndex === 1 ? 2 : 1;
            const $newColumn = $question.find('.column-dropzone[data-column-index="' + newColumnIndex + '"]');
            
            // Move the item
            $item.detach().appendTo($newColumn);
            ...
            // CRITICAL: Update the answer
            updateColumnSortingAnswer($question);
            
            // Provide feedback
            const itemText = $item.data('item-text') || $item.find('.item-text').text();
            const columnTitle = $newColumn.siblings('.column-title').text() || 'Column ' + newColumnIndex;
            const message = isGreek ? 
                `${itemText} μετακινήθηκε στη στήλη ${columnTitle}` : 
                `${itemText} moved to column ${columnTitle}`;
            announceToScreenReader(message);
    ...
    function displayResults(resultData) {
        ...
        const associatedLessonId = resultData.associated_lesson_id || null; // Need to pass this from backend
        const associatedLessonUrl = resultData.associated_lesson_url || null; // Need to pass this from backend
        const associatedLessonTitle = resultData.associated_lesson_title || null; // Need to pass this from backend
        ...
        if (!passed) {
            // Add both retry quiz and review lesson buttons for failed attempts
            html += '<div class="failed-quiz-actions">';
            
            // Retry Quiz button
            html += '<button type="button" class="retry-btn" onclick="location.reload()">' + 
                (strings.retry_quiz || 'Επανάληψη Κουίζ') + '</button>';
            
            // Review Lesson button (if associated lesson exists) - NOW AS A BUTTON
            if (associatedLessonUrl) {
                html += '<button type="button" class="retry-btn" onclick="window.location.href=\'' + associatedLessonUrl + '\'">' + 
                    (strings.review_lesson || 'Επανάληψη Άρθρου') + '</button>';
            }
            
            html += '</div>';
            
            // Optional: Add a helpful message
            if (associatedLessonTitle) {
                html += '<p class="review-suggestion">';
                html += (isGreek ? 
                    'Προτείνουμε να επαναλάβετε το άρθρο <strong>"' + associatedLessonTitle + '"</strong> πριν δοκιμάσετε ξανά το κουίζ.' :
                    'We suggest reviewing the lesson <strong>"' + associatedLessonTitle + '"</strong> before retrying the quiz.');
                html += '</p>';
            }
        } else if (passed && associatedLessonUrl) {
            // For passed quizzes, also use button format
            html += '<div class="passed-quiz-actions">';
            html += '<button type="button" class="review-lesson-btn secondary" onclick="window.location.href=\'' + associatedLessonUrl + '\'">' + 
                '<i class="fas fa-book-open"></i> ' +
                (isGreek ? 'Δείτε Ξανά το Άρθρο' : 'Review Lesson') + '</button>';
            html += '</div>';
        }
        
        html += '</div>'
        
        html += '</div>'; // Close quiz-result-actions
        html += '</div>'; // Close quiz-results
        
        // Add custom styles for the new buttons
        html += '<style>';
        html += '.quiz-result-actions { margin-top: 30px; text-align: center; }';
        html += '.failed-quiz-actions { display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; }';
        html += '.passed-quiz-actions { display: flex; gap: 15px; justify-content: center; }';
        
        // Retry button styles (existing)
        html += '.retry-btn { background: #3b82f6; color: white; border: none; padding: 16px 32px; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 8px; }';
        html += '.retry-btn:hover { background: #2563eb; transform: translateY(-2px); box-shadow: 0 6px 16px rgba(59, 130, 246, 0.3); }';
        
        // Review lesson button styles (new)
        html += '.review-lesson-btn { background: #10b981; color: white; border: none; padding: 16px 32px; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }';
        html += '.review-lesson-btn:hover { background: #059669; transform: translateY(-2px); box-shadow: 0 6px 16px rgba(16, 185, 129, 0.3); color: white; text-decoration: none; }';
        html += '.review-lesson-btn.secondary { background: #6b7280; }';
        html += '.review-lesson-btn.secondary:hover { background: #4b5563; box-shadow: 0 6px 16px rgba(107, 114, 128, 0.3); }';
        
        // Review suggestion text
        html += '.review-suggestion { margin-top: 20px; padding: 15px; background: #fef3c7; border: 1px solid #fbbf24; border-radius: 8px; color: #92400e; font-size: 14px; }';
        html += '.review-suggestion strong { color: #78350f; }';
        
        // Mobile responsive
        html += '@media (max-width: 640px) {';
        html += '  .failed-quiz-actions { flex-direction: column; align-items: stretch; }';
        html += '  .retry-btn, .review-lesson-btn { width: 100%; justify-content: center; }';
        html += '}';
    ...
    function displayUserAnswers(detailedResults, isPerfectScore = false) {
        console.log('=== DISPLAY USER ANSWERS ===');
        console.log('detailedResults received:', detailedResults);
        
        if (detailedResults && detailedResults.length > 0) {
            console.log('First question object:', detailedResults[0]);
            console.log('First question text:', detailedResults[0].question);
        }
        ...
        detailedResults.forEach((result, index) => {
            ...
            // THIS IS THE KEY LINE - Display the actual question text
    ...
    // Also check displayCorrectAnswers() function
