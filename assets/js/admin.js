...
jQuery(document).ready(function($) {
    ...
    // Add Column 1 Item
    $(document).on('click', '.add-column1-item', function(e) {
        e.preventDefault();
        
        var button = $(this);
        var container = button.prev('.column1-items-container');
        var questionDiv = button.closest('.quiz-question');
        var questionIdx = questionDiv.data('index');
        
        // Count existing items to determine next index
        var itemIndex = container.find('.column-item').length;
        
        var newItem = '<div class="column-item">' +
            '<input type="text" name="quiz_questions[' + questionIdx + '][column1_items][' + itemIndex + ']" placeholder="Στοιχείο στήλης 1" class="regular-text" />' +
            '<button type="button" class="remove-column1-item button-link-delete">' + strings.remove + '</button>' +
            '</div>';
    ...
    // Add Column 2 Item
    $(document).on('click', '.add-column2-item', function(e) {
        e.preventDefault();
        
        var button = $(this);
        var container = button.prev('.column2-items-container');
        var questionDiv = button.closest('.quiz-question');
        var questionIdx = questionDiv.data('index');
        ...
        var newItem = '<div class="column-item">' +
            '<input type="text" name="quiz_questions[' + questionIdx + '][column2_items][' + itemIndex + ']" placeholder="Στοιχείο στήλης 2" class="regular-text" />' +
            '<button type="button" class="remove-column2-item button-link-delete">' + strings.remove + '</button>' +
            '</div>';
    ...
    $(document).on('click', '.remove-column1-item', function(e) {
        e.preventDefault();
        var container = $(this).closest('.column1-items-container');
        var questionDiv = $(this).closest('.quiz-question');
        var questionIdx = questionDiv.data('index');
        ...
        // Re-index remaining items
        container.find('.column-item').each(function(index) {
            $(this).find('input[type="text"]').attr('name', 'quiz_questions[' + questionIdx + '][column1_items][' + index + ']');
        });
    });
    
    // Remove Column 2 Item
    $(document).on('click', '.remove-column2-item', function(e) {
        e.preventDefault();
        var container = $(this).closest('.column2-items-container');
        var questionDiv = $(this).closest('.quiz-question');
        var questionIdx = questionDiv.data('index');
        ...
        // Re-index remaining items
        container.find('.column-item').each(function(index) {
            $(this).find('input[type="text"]').attr('name', 'quiz_questions[' + questionIdx + '][column2_items][' + index + ']');
        });
    });
    ...
    function loadQuestionTypeOptions(questionType, questionIdx, container) {
        ...
        switch (questionType) {
            ...
            case 'yes_no':  // ADD THIS NEW CASE
                html = '<h5>' + strings.correct_answer + '</h5>' +
                    '<label><input type="radio" name="quiz_questions[' + questionIdx + '][correct_answer_yes_no]" value="yes" checked /> ' + (isGreek ? 'Ναι' : 'Yes') + '</label><br>' +
                    '<label><input type="radio" name="quiz_questions[' + questionIdx + '][correct_answer_yes_no]" value="no" /> ' + (isGreek ? 'Όχι' : 'No') + '</label>';
                break;
            ...
            case 'column_sorting':  // ADD THIS NEW CASE
                html = '<div class="column-sorting-setup">' +
                    '<h5>Ρυθμίσεις Στηλών</h5>' +
                    '<table class="form-table">' +
                    '<tr>' +
                    '<th><label>Τίτλος Στήλης 1</label></th>' +
                    '<td><input type="text" name="quiz_questions[' + questionIdx + '][column1_title]" placeholder="π.χ. Ευρώπη" class="regular-text" /></td>' +
                    '</tr>' +
                    '<tr>' +
                    '<th><label>Τίτλος Στήλης 2</label></th>' +
                    '<td><input type="text" name="quiz_questions[' + questionIdx + '][column2_title]" placeholder="π.χ. Ασία" class="regular-text" /></td>' +
                    '</tr>' +
                    '<tr>' +
                    '<th><label>Τύπος Ανακάτεμα</label></th>' +
                    '<td>' +
                    '<select name="quiz_questions[' + questionIdx + '][shuffle_type]">' +
                    '<option value="random">Τυχαία κατανομή</option>' +
                    '<option value="all_wrong">Όλα σε λάθος στήλη</option>' +
                    '</select>' +
                    ...
                    '<input type="text" name="quiz_questions[' + questionIdx + '][column1_items][0]" placeholder="Στοιχείο στήλης 1" class="regular-text" />' +
                    '<button type="button" class="remove-column1-item button-link-delete">' + strings.remove + '</button>' +
                    '</div>' +
                    '</div>' +
                    ...
                    '<div class="column-item">' +
                    '<input type="text" name="quiz_questions[' + questionIdx + '][column2_items][0]" placeholder="Στοιχείο στήλης 2" class="regular-text" />' +
                    '<button type="button" class="remove-column2-item button-link-delete">' + strings.remove + '</button>' +
                    '</div>' +
                    '</div>' +
                    ...
                    '</div>' +
                    '<style>' +
                    '.column-sorting-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px; }' +
                    '.column-items { border: 1px solid #ccc; padding: 15px; border-radius: 4px; background: #f9f9f9; }' +
                    '.column-items h5 { margin-top: 0; }' +
