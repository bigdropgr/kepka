...
class ELearning_Ajax {
    ...
    public function submitQuiz(): void {
        ...
        // Save the answer WITH THE QUESTION TEXT
        ELearning_Database::saveQuizAnswer(
            $attempt_id,
            $question_index,
            $question['type'],
            $question['question'],  // THIS MUST BE THE ACTUAL QUESTION TEXT
            $user_answer_for_db,
            $correct_answer,
            $is_correct,
            $time_spent
        ...
        // ADD THIS: Associated lesson information (for review when failed)
        $associated_lesson_id = get_post_meta($quiz_id, '_associated_lesson', true);
        if ($associated_lesson_id) {
            $lesson = get_post($associated_lesson_id);
            if ($lesson && $lesson->post_status === 'publish') {
                $response_data['associated_lesson_id'] = $associated_lesson_id;
                $response_data['associated_lesson_url'] = get_permalink($associated_lesson_id);
                $response_data['associated_lesson_title'] = $lesson->post_title;
            }
        }
        
        // Add next lesson info if quiz is passed
        if ($results['passed']) {
            // Get associated lesson for this quiz
            $associated_lesson_id = get_post_meta($quiz_id, '_associated_lesson', true);
            
            if ($associated_lesson_id) {
                // Get next lesson
                $next_lesson = $this->getNextLesson($associated_lesson_id);
        ...
        // Debug Point 3: Right before sending response (around line 280)
        if ($show_results === 'yes') {
        $response_data['detailed_results'] = $this->getDetailedResults($questions, $user_answers);
        
        error_log('=== DEBUG: Response Data Being Sent ===');
        error_log('Number of detailed results: ' . count($response_data['detailed_results']));
        
        // Check first question in response
        if (!empty($response_data['detailed_results'][0])) {
            $first = $response_data['detailed_results'][0];
            error_log('First result question field exists: ' . (isset($first['question']) ? 'YES' : 'NO'));
            error_log('First result question preview: ' . substr($first['question'] ?? '', 0, 50));
        }
    }
    ...
    private function getQuestionMaxPoints($question): int {
        switch ($question['type']) {
            ...
            case 'column_sorting':
                $column1_items = $question['column1_items'] ?? [];
                $column2_items = $question['column2_items'] ?? [];
                // Total points = total unique items (remove duplicates between columns)
                $all_items = array_unique(array_merge($column1_items, $column2_items));
    ...
    private function calculateQuestionPoints($question, $user_answer): array {
        switch ($question['type']) {
            ...
            case 'yes_no':  // ADD THIS CASE
                return $this->calculateYesNoPoints($question, $user_answer);
            ...
            case 'column_sorting':
                return $this->calculateColumnSortingPoints($question, $user_answer);
    ...
    /**
     * Calculate points for yes/no questions
     */
    private function calculateYesNoPoints($question, $user_answer): array {
    ...
    /**
     * Calculate points for column sorting questions WITH PENALTIES
     */
    private function calculateColumnSortingPoints($question, $user_answer): array {
        // Get correct answers
        $correct_column1 = $question['column1_items'] ?? [];
        $correct_column2 = $question['column2_items'] ?? [];
        
        // Get user's answers
        if (!is_array($user_answer) || !isset($user_answer['column1']) || !isset($user_answer['column2'])) {
            return ['max_points' => 1, 'earned_points' => 0];
        }
        
        $user_column1 = $user_answer['column1'] ?? [];
        $user_column2 = $user_answer['column2'] ?? [];
        
        // Get all unique items
        $all_items = array_unique(array_merge($correct_column1, $correct_column2));
        $max_points = count($all_items);
        
        $points = 0;
        $debug_info = [];
        
        // Check each item's placement
        foreach ($all_items as $item) {
            $should_be_in_1 = in_array($item, $correct_column1);
            $should_be_in_2 = in_array($item, $correct_column2);
            $is_in_1 = in_array($item, $user_column1);
            $is_in_2 = in_array($item, $user_column2);
            
            if ($should_be_in_1 && $should_be_in_2) {
                // Item can be in either column (like Istanbul example)
                if ($is_in_1 || $is_in_2) {
                    $points++; // Correct placement
                    $debug_info[] = "$item: Can be in both, placed correctly (+1)";
                } else {
                    // Item not placed anywhere
                    $debug_info[] = "$item: Can be in both, not placed (0)";
                }
            } elseif ($should_be_in_1) {
                // Item should only be in column 1
                if ($is_in_1 && !$is_in_2) {
                    $points++; // Correct placement
                    $debug_info[] = "$item: Should be in column 1, placed correctly (+1)";
                } elseif ($is_in_2 && !$is_in_1) {
                    $points--; // Wrong column - PENALTY
                    $debug_info[] = "$item: Should be in column 1, placed in column 2 (-1)";
                } else {
                    // Not placed or in both (shouldn't happen)
                    $debug_info[] = "$item: Should be in column 1, not placed correctly (0)";
                }
            } elseif ($should_be_in_2) {
                // Item should only be in column 2
                if ($is_in_2 && !$is_in_1) {
                    $points++; // Correct placement
                    $debug_info[] = "$item: Should be in column 2, placed correctly (+1)";
                } elseif ($is_in_1 && !$is_in_2) {
                    $points--; // Wrong column - PENALTY
                    $debug_info[] = "$item: Should be in column 2, placed in column 1 (-1)";
                } else {
                    // Not placed or in both (shouldn't happen)
                    $debug_info[] = "$item: Should be in column 2, not placed correctly (0)";
                }
            }
        }
        
        // Log for debugging
        error_log("Column Sorting Scoring:");
        foreach ($debug_info as $info) {
            error_log("  " . $info);
        }
        error_log("  Total: $points / $max_points points");
    ...
    private function getCorrectAnswer($question) {
        switch ($question['type']) {
            ...
            case 'yes_no':  // ADD THIS CASE
                return $question['correct_answer_yes_no'] ?? 'yes';
            ...
            case 'column_sorting':
                return [
                    'column1' => $question['column1_items'] ?? [],
                    'column2' => $question['column2_items'] ?? []
                ];
    ...
    private function getDetailedResults($questions, $user_answers): array {
        ...
        error_log('=== DEBUG: Building Detailed Results ===');
        
        foreach ($questions as $question_index => $question) {
            // Skip invalid questions
            if (empty($question['type']) || empty($question['question'])) {
                error_log("Skipping question $question_index - missing type or question text");
                continue;
            }
            
            error_log("Processing question $question_index:");
            error_log("  Question text: " . substr($question['question'], 0, 100));
            ...
            // CRITICAL: Store the actual question text, not stripped
            $result_data = [
                'question' => $question['question'],  // Keep the raw question text with HTML
            ...
            } elseif ($question['type'] === 'column_sorting') {
                $result_data['column1_title'] = $question['column1_title'] ?? 'Στήλη 1';
                $result_data['column2_title'] = $question['column2_title'] ?? 'Στήλη 2';
                $result_data['column1_items'] = $question['column1_items'] ?? [];
                $result_data['column2_items'] = $question['column2_items'] ?? [];
            }
        ...
        error_log('Total detailed results: ' . count($detailed_results));
    ...
    private function formatAnswerForDisplay($answer, $question): string {
        ...
        switch ($question['type']) {
            ...
            case 'yes_no':  // ADD THIS NEW CASE
                if ($answer === 'yes') {
                    return __('Ναι', 'elearning-quiz');
                } elseif ($answer === 'no') {
                    return __('Όχι', 'elearning-quiz');
                }
                return __('Δεν δόθηκε απάντηση', 'elearning-quiz');
            ...
            case 'column_sorting':
                if (is_array($answer)) {
                    ...
                    if (isset($answer['column1']) && !empty($answer['column1'])) {
                        $column1_title = $question['column1_title'] ?? 'Στήλη 1';
                        $display[] = $column1_title . ': ' . implode(', ', $answer['column1']);
                    }
                    
                    if (isset($answer['column2']) && !empty($answer['column2'])) {
                        $column2_title = $question['column2_title'] ?? 'Στήλη 2';
                        $display[] = $column2_title . ': ' . implode(', ', $answer['column2']);
                    }
                    
                    return !empty($display) ? implode(' | ', $display) : __('Δεν έγινε ταξινόμηση', 'elearning-quiz');
                }
                return __('Δεν έγινε ταξινόμηση', 'elearning-quiz');
    ...
    public function updateLessonProgress(): void {
        ...
        if (!$lesson_id) {
            wp_send_json_error(__('Μη έγκυρο ID άρθρου', 'elearning-quiz'));
        }
        
        // Validate lesson exists
        $lesson = get_post($lesson_id);
        if (!$lesson || $lesson->post_type !== 'elearning_lesson') {
            wp_send_json_error(__('Το άρθρο δεν βρέθηκε', 'elearning-quiz'));
        }
    ...
    /**
     * Get next lesson helper
     */
    private function getNextLesson($current_lesson_id) {
        $current_lesson = get_post($current_lesson_id);
        if (!$current_lesson) {
            return null;
        }
        
        // Get lessons in the same category
        $categories = wp_get_post_terms($current_lesson_id, 'lesson_category', ['fields' => 'ids']);
        
        $args = [
            'post_type' => 'elearning_lesson',
            'posts_per_page' => 1,
            'post_status' => 'publish',
            'post__not_in' => [$current_lesson_id],
            'orderby' => 'date',
            'order' => 'ASC',
            'date_query' => [
                [
                    'after' => $current_lesson->post_date,
                    'inclusive' => false
                ]
            ]
        ];
        
        // If lesson has categories, get next lesson from same category
        if (!empty($categories)) {
            $args['tax_query'] = [
                [
        ...
        if (!empty($next_lessons)) {
            return $next_lessons[0];
        }
        
        // If no next lesson in category, get any next lesson
        unset($args['tax_query']);
        $next_lessons = get_posts($args);
        
        return !empty($next_lessons) ? $next_lessons[0] : null;
    }
}