...
class ELearning_Frontend {
        ...
        if ($associated_lesson) {
            ...
            if (!$lesson_completed) {
                ...
                if ($lesson && $lesson->post_status === 'publish') {
                    ...
                    $content .= '<p>Πρέπει να ολοκληρώσετε το άρθρο <strong>' . esc_html($lesson->post_title) . '</strong> για να μπορέσετε να δώσετε αυτό το κουίζ.</p>';
    ...
    public function addLessonContent($content) {
        ...
        if (empty($sections)) {
            ...
            $content .= '<p>Αυτό το άρθρο δεν έχει ενότητες ακόμα.</p>';
            $content .= '</div>';
            return $content;
        }
    ...
    private function renderQuestion($original_index, $question, $display_index = null) {
        ...
        switch ($question['type']) {
            ...
            case 'true_false':
                ...
                break;
            case 'yes_no':
                $html .= $this->renderYesNo($original_index, $question);
                break;
            case 'matching':
                $html .= $this->renderMatching($original_index, $question);
                break;
            case 'column_sorting':  // ADD THIS CASE
                $html .= $this->renderColumnSorting($original_index, $question);
                break;
        }
    ...
    private function renderFillBlanks($index, $question) {
        ...
        $html = '<p class="instruction">Τοποθετείστε τις λέξεις για να συμπληρώσετε τα κενά:</p>';
    ...
    private function renderTrueFalse($index, $question) {
        // Create a truly unique name for this specific question's radio group
        $unique_id = 'tf_' . $index . '_' . uniqid();
        ...
        $html .= '<input type="radio" name="questions[' . $index . '][answer]" value="true" data-question-index="' . $index . '" />';
        $html .= '<span class="option-text">Σωστό</span>';
        $html .= '</label>';
        
        $html .= '<label class="option-label neutral-option">';
        $html .= '<input type="radio" name="questions[' . $index . '][answer]" value="false" data-question-index="' . $index . '" />';
     ...
     */
    private function renderYesNo($index, $question) {
        $html = '<p class="instruction">Επιλέξτε Ναι ή Όχι:</p>';
        
        $html .= '<div class="yes-no-options">';
        
        $html .= '<label class="option-label neutral-option">';
        $html .= '<input type="radio" name="questions[' . $index . '][answer]" value="yes" />';
        $html .= '<span class="option-text">Ναι</span>';
        $html .= '</label>';
        
        $html .= '<label class="option-label neutral-option">';
        $html .= '<input type="radio" name="questions[' . $index . '][answer]" value="no" />';
        $html .= '<span class="option-text">Όχι</span>';
        $html .= '</label>';
    ...
    private function renderMatching($index, $question) {
        ...
        // CHANGED: Updated instruction text from "Σύρετε" to "Τοποθετήστε"
        $html = '<p class="instruction">Τοποθετήστε τα στοιχεία από τη δεξιά στήλη για να ταιριάξουν με τα στοιχεία στην αριστερή στήλη:</p>';
        
        // Add a mobile-specific instruction if needed
        $html .= '<p class="instruction mobile-instruction" style="display: none;">Πατήστε σε ένα στοιχείο από τη δεξιά στήλη και μετά πατήστε στη θέση που θέλετε να το τοποθετήσετε στην αριστερή στήλη:</p>';
        ...
        foreach ($left_column as $left_index => $left_item) {
            ...
            $html .= '<span class="drop-placeholder">Τοποθετήστε εδώ</span>'; // CHANGED: From "Αφήστε εδώ" to "Τοποθετήστε εδώ"
            $html .= '</div>';
        ...
        // Add CSS to show/hide instructions based on device
        $html .= '<style>
        @media (max-width: 768px) {
            .matching-container .instruction:not(.mobile-instruction) {
                display: none !important;
            }
            .matching-container .mobile-instruction {
    ...
    /**
     * Render column sorting question with shuffled items
     */
    private function renderColumnSorting($index, $question) {
        $column1_title = $question['column1_title'] ?? 'Column 1';
        $column2_title = $question['column2_title'] ?? 'Column 2';
        $column1_items = $question['column1_items'] ?? [];
        $column2_items = $question['column2_items'] ?? [];
        $shuffle_type = $question['shuffle_type'] ?? 'random';
        
        // Combine all items for shuffling
        $all_items = [];
        foreach ($column1_items as $item) {
            $all_items[] = ['text' => $item, 'correct_column' => 1];
        }
        foreach ($column2_items as $item) {
            // Check if item exists in column1 too (can be in both)
            $in_both = in_array($item, $column1_items);
            $all_items[] = ['text' => $item, 'correct_column' => $in_both ? 'both' : 2];
        }
        
        // Shuffle items based on shuffle type
        $column1_display = [];
        $column2_display = [];
        
        if ($shuffle_type === 'all_wrong') {
            // Put all items in the wrong column
            foreach ($all_items as $item) {
                if ($item['correct_column'] === 1) {
                    $column2_display[] = $item;
                } elseif ($item['correct_column'] === 2) {
                    $column1_display[] = $item;
                } else { // both
                    // Randomly assign items that can be in both
                    if (rand(0, 1) === 0) {
                        $column1_display[] = $item;
                    } else {
                        $column2_display[] = $item;
        ...
        shuffle($column1_display);
        shuffle($column2_display);
        
        $html = '<p class="instruction">Τοποθετείστε τα στοιχεία στη σωστή στήλη:</p>';
        
        $html .= '<div class="column-sorting-container">';
        
        // Column 1
        $html .= '<div class="sorting-column" data-column="1">';
        $html .= '<h4 class="column-title">' . esc_html($column1_title) . '</h4>';
        $html .= '<div class="column-dropzone sortable-column" data-column-index="1">';
        
        foreach ($column1_display as $item_index => $item) {
            $html .= '<div class="sortable-item" draggable="true" data-item-text="' . esc_attr($item['text']) . '" data-original-column="' . esc_attr($item['correct_column']) . '">';
            $html .= '<span class="item-text">' . esc_html($item['text']) . '</span>';
        ...
        // Column 2
        $html .= '<div class="sorting-column" data-column="2">';
        $html .= '<h4 class="column-title">' . esc_html($column2_title) . '</h4>';
        $html .= '<div class="column-dropzone sortable-column" data-column-index="2">';
        
        foreach ($column2_display as $item_index => $item) {
            $html .= '<div class="sortable-item" draggable="true" data-item-text="' . esc_attr($item['text']) . '" data-original-column="' . esc_attr($item['correct_column']) . '">';
            $html .= '<span class="item-text">' . esc_html($item['text']) . '</span>';
            $html .= '<span class="drag-handle">⋮⋮</span>';
            $html .= '</div>';
        ...
        // Hidden inputs to store the answer
        $html .= '<input type="hidden" name="questions[' . $index . '][column1_answer]" value="" class="column-sorting-answer" data-column="1" />';
        $html .= '<input type="hidden" name="questions[' . $index . '][column2_answer]" value="" class="column-sorting-answer" data-column="2" />';
        
        // Store correct answers as data attributes for validation
        $html .= '<div class="column-sorting-correct-data" style="display:none;" ';
        $html .= 'data-column1-correct=\'' . json_encode($column1_items) . '\' ';
        $html .= 'data-column2-correct=\'' . json_encode($column2_items) . '\'>';
        $html .= '</div>';
        
        // Add CSS for column sorting
        $html .= '<style>
        .column-sorting-container {
            display: grid;
    ...
    /**
     * Get next lesson in the same category or by menu order
     */
    private function getNextLesson($current_lesson_id) {
        // First, try to get the next lesson by menu order
        ...
        $args = [
            ...
            'meta_query' => [
                ...
                [
                    'key' => 'menu_order',
                    'value' => $current_lesson->menu_order,
                    'compare' => '>',
                    'type' => 'NUMERIC'
                ],
                [
                    'key' => 'post_date',
                    'value' => $current_lesson->post_date,
                    'compare' => '>',
                    'type' => 'DATETIME'
                ]
            ]
